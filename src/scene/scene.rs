...
impl Scene {
    ...
    pub fn new_from_cad(cad_data: &CADData) -> Result<Self> {
        ...
        let (geometries, instances) = global_state.to_geometries_and_instances();
...
impl GlobalTraversalState {
    ...
    /// Transforms and returns the geometries and instances.
    pub fn to_geometries_and_instances(self) -> (Vec<Geometry>, Vec<Instance>) {
        let geometries = self.geometries;
