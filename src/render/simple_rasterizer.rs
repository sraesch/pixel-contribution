...
impl SimpleRasterizer {
    ...
    fn rasterize_and_dequantize_page<Integer: IntegerTrait>(
        ...
        let size = self.frame_buffer.get_frame_size() as f32;
        ...
        for (triangle, local_id) in triangles.iter().zip(local_ids.iter()) {
            ...
            let p0 = Self::project_pos(size, &m, &p0);
            let p1 = Self::project_pos(size, &m, &p1);
            let p2 = Self::project_pos(size, &m, &p2);

            self.frame_buffer.rasterize(id, &p0, &p1, &p2);
        }
    }
...
impl Renderer for SimpleRasterizer {
    ...
    fn render_frame(
        ...
        pages.iter().for_each(|page| {
            self.rasterize_page(page, &pmmat);
